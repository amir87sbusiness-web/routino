...
describe("lifecycle sync scheduler", () => {
  ...
  it("uses a trailing thirty-second edit window", async () => {
  ...
  it("coalesces redundant clean foreground pulls for three minutes", async () => {
    hasPending.mockResolvedValue(false);
    const scheduler = makeScheduler();
  ...
  it("skips a clean boot pull when the shared cursor was synced within two minutes", async () => {
    hasPending.mockResolvedValue(false);
