...
const env = loadEnv({
  ...
  PSP_PROVIDER: "fake",
  PROXY_SECRET: "",
  JWT_SECRET: "local-audit-jwt-value-".repeat(3),
  OTP_PEPPER: "local-audit-otp-value-".repeat(3),
  ADMIN_PHONE: "09120000123",
  ADMIN_SESSION_SECRET: "local-audit-admin-".repeat(3),
  PUBLIC_API_URL: "http://audit.invalid",
  PUBLIC_WEB_URL: "http://audit.invalid/app",
});
...
afterAll(async () => {
  ...
  writeFileSync(
    resolve(out, "auth-payment-results.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        driver: "postgres-js; prepare:false",
        database: "fresh loopback-only PostgreSQL; synthetic data; no external provider calls",
        runtime: process.version,
        defaultPoolMax: 2,
        limitations:
          "Hono routes under Node, not deployed Deno, Supabase gateway, Cloudflare, bank or SMS network. Burst timing is local measured latency, not production capacity certification.",
...
async function reset() {
  await client
    .unsafe(
      `truncate users, records, otp_codes, provider_capacity_leases,
    auth_rate_limit_buckets, discounts, redemptions, payments, grants, entitlements,
    feedback, anonymous_counters restart identity cascade`,
    )
    .simple();
  ...
  sent = [];
  fake = fakePsp("http://audit.invalid");
  clockMs = Date.now();
  queryCount = 0;
  smsDelay = requestDelay = verifyDelay = 0;
  smsActive = smsPeak = requestActive = requestPeak = verifyActive = verifyPeak = 0;
  requests = verifies = 0;
  smsFailure = false;
  requestOutcome = "issued";
  verifyOutcome = "normal";
  env.SMS_PROVIDER_MAX_CONCURRENCY = 32;
  env.PSP_PROVIDER_MAX_CONCURRENCY = 64;
}

async function seed(n: number) {
  const users = Array.from({ length: n }, (_, i) => ({
    id: randomUUID(),
    phone: `98912${String(8800000 + i)}`,
...
async function wave(name: string, work: (() => Promise<Response>)[], metadata = {}) {
  ...
  const samples = await Promise.all(
    work.map(async (fn) => {
      ...
      try {
        ...
        return { status: 0, ms: performance.now() - t, text: String(error), retryAfter: null };
      }
    }),
  );
  clearInterval(timer);
  ...
  records.push({
    ...
    statusCounts: samples.reduce<Record<number, number>>((counts, s) => {
      counts[s.status] = (counts[s.status] ?? 0) + 1;
      return counts;
    }, {}),
    queryCount: queryCount - beforeQueries,
    queryCountMeaning:
      "client SQL statements logged by Drizzle; excludes SQL internal work; includes transaction controls",
    cpuMs: +((cpuDelta.user + cpuDelta.system) / 1000).toFixed(2),
    peakNodeRssMiB: +(peakRss / 1024 ** 2).toFixed(2),
    ...metadata,
  });
  return samples;
}
...
test("50 simultaneous password logins with Edge-sized max:2 pool", async () => {
  await reset();
  const people = await seed(50);
  const responses = await wave(
    "password_login_50_distinct_users",
    people.map(
      (u) => () =>
        call(
...
test("50 concurrent OTP requests, safe retry, then 50 concurrent OTP logins", async () => {
  ...
  const people = await seed(50);
  smsDelay = 750;
  const request = (u: (typeof people)[number]) => () =>
    call("/v1/auth/otp/request", { phone: u.phone }, { "x-forwarded-for": u.ip });
  const first = await wave("otp_request_50_distinct_ips_sms750ms", people.map(request), {
    poolMax: 2,
    smsLimit: 32,
    mockSmsDelayMs: 750,
  });
  ...
  if (retryPeople.length) {
    const retries = await wave("otp_retry_after_capacity_release", retryPeople.map(request), {
      poolMax: 2,
    });
  ...
  const logins = await wave(
    "otp_verify_50_distinct_users",
    people.map(
      (u) => () =>
        call(
          "/v1/auth/otp/verify",
          { phone: u.phone, code: codes.get(u.phone) },
          { "x-forwarded-for": u.ip },
        ),
    ),
    { poolMax: 2 },
  ...
  expect(
    (await client`select count(*)::int as n from otp_codes where consumed_at is not null`)[0]!.n,
  ).toBe(50);
...
test("50 OTP requests from a shared NAT IP and same-code replay diagnostic", async () => {
  ...
  const people = await seed(50);
  await wave(
    "otp_request_50_shared_nat",
    people.map(
      (u) => () =>
        call("/v1/auth/otp/request", { phone: u.phone }, { "x-forwarded-for": "203.0.113.10" }),
    ),
    { poolMax: 2, ipLimitPerHour: 20 },
  );
  records.push({
    name: "otp_shared_nat_ledger",
    sent: sent.length,
    count: (await client`select count(*)::int as n from otp_codes`)[0]!.n,
  });
  ...
  const replay = await wave(
    "otp_same_code_50_replays",
    Array.from(
      { length: 50 },
      () => () => call("/v1/auth/otp/verify", { phone: user!.phone, code: sent[0]!.code }),
    ),
    { poolMax: 2, intendedInvariant: "exactly one successful use of a one-time code" },
  );
  records.push({
    name: "otp_replay_invariant",
    accepted: replay.filter((r) => r.status === 200).length,
    rows: await client`select attempts, consumed_at is not null as consumed from otp_codes`,
  });
...
test("20 simultaneous purchases, duplicate checkouts and 100 callback deliveries", async () => {
  await reset();
  const people = await seed(20);
  requestDelay = 250;
  verifyDelay = 250;
  const attempts = people.map(() => randomUUID());
  const work = people.map(
    (u, i) => () =>
      call("/v1/payments/checkout", { planId: "m1", attemptId: attempts[i] }, auth(u.access)),
  );
  const responses = await wave("checkout_20_distinct_buyers_provider250ms", work, {
    poolMax: 2,
  ...
  await wave(
    "callback_100_deliveries_20_paid_orders",
    orders.flatMap((order) =>
      Array.from(
        { length: 5 },
        () => () =>
          call(
            `/v1/payments/callback?paymentId=${order.paymentId}&Authority=${order.authority}&Status=OK`,
          ),
      ),
    ),
    { poolMax: 2, duplicateCallbacksPerOrder: 5 },
  );
  const ledger =
    await client`select count(*)::int as payments, count(*) filter (where status='paid' and applied_at is not null)::int as paid from payments`;
  const grants =
    await client`select count(*)::int as grants, count(distinct payment_id)::int as unique_payments, count(distinct user_id)::int as unique_users from grants where source='payment'`;
  records.push({
    name: "payment_success_invariants",
    ledger,
    grants,
    providerRequests: requests,
    providerVerifies: verifies,
    peakCreateCalls: requestPeak,
    peakVerifyCalls: verifyPeak,
  });
  expect(ledger[0]!.paid).toBe(20);
  expect(grants[0]!.grants).toBe(20);
  expect(grants[0]!.unique_users).toBe(20);
  expect(verifies).toBe(20);
}, 60_000);
...
test("20 purchases with provider timeout/rejection; ambiguous attempts are never resent", async () => {
  ...
  const people = await seed(20);
  requestDelay = 250;
  requestOutcome = "mixed";
  const work = people.map((u) => {
    const attemptId = randomUUID();
    return () => call("/v1/payments/checkout", { planId: "m1", attemptId }, auth(u.access));
  });
  const first = await wave("checkout_20_mixed_provider_results", work, {
    poolMax: 2,
    mockDelayMs: 250,
    pattern: "10 issued / 5 rejected / 5 ambiguous",
  });
  ...
  expect(requests).toBe(20);
  const states =
    await client`select status, count(*)::int as n from payments group by status order by status`;
  records.push({
    name: "ambiguous_payment_invariants",
    states,
    providerRequests: requests,
    grants: (await client`select count(*)::int as n from grants`)[0]!.n,
  });
  expect((await client`select count(*)::int as n from grants`)[0]!.n).toBe(0);
}, 60_000);
...
test("PSP verify capacity and unknown callback backoff diagnostics", async () => {
  await reset();
  const people = await seed(20);
  const orders: { paymentId: string; authority: string }[] = [];
  for (const user of people) {
    orders.push(
      ...
      ).json()) as { paymentId: string; authority: string },
    );
  }
  env.PSP_PROVIDER_MAX_CONCURRENCY = 1;
  verifyDelay = 250;
  verifyOutcome = "unknown";
  const callback = (order: { paymentId: string; authority: string }) => () =>
    call(
      `/v1/payments/callback?paymentId=${order.paymentId}&Authority=${order.authority}&Status=OK`,
    );
  await wave("verify_20_with_configured_psp_capacity1", orders.map(callback), {
    poolMax: 2,
    configuredPspCapacity: 1,
    providerDelayMs: 250,
  });
  const peak = verifyPeak;
  ...
  await wave(
    "poll_20_during_verify_backoff",
    people.map(
      (u, i) => () => call(`/v1/payments/${orders[i].paymentId}`, undefined, auth(u.access)),
    ),
    { poolMax: 2 },
  );
  records.push({
    name: "verify_capacity_backoff_diagnostic",
    configuredPspCapacity: 1,
    observedPeakVerifyCalls: peak,
    repeatCallbacks: 5,
    additionalVerifies: afterCallbacks - before,
    subsequentPollVerifies: verifies - afterCallbacks,
    grants: (await client`select count(*)::int as n from grants`)[0]!.n,
  });
  expect((await client`select count(*)::int as n from grants`)[0]!.n).toBe(0);
  ...
  for (let i = 0; i < people.length; i++) {
    expect(
      (await call(`/v1/payments/${orders[i].paymentId}`, undefined, auth(people[i]!.access)))
        .status,
    ).toBe(200);
  }
  expect((await client`select count(*)::int as n from grants where source='payment'`)[0]!.n).toBe(
    20,
  );
  expect(
    (await client`select count(*)::int as n from provider_capacity_leases where kind='psp'`)[0]!.n,
  ).toBe(0);
}, 60_000);
...
test("real PostgreSQL statement snapshot diagnostic under 20 locked concurrent OTP claims", async () => {
  ...
  const pending = Array.from({ length: 20 }, () =>
    claimSendSlot(db, env, phone, null, new Date(clockMs)),
  );
  let observedWaiting = 0;
  try {
    for (let i = 0; i < 100; i++) {
      const [row] =
        await monitor`select count(*)::int as n from pg_stat_activity where datname='auth_payment_audit' and wait_event='advisory'`;
  ...
  records.push({
    name: "otp_real_pg_lock_snapshot_diagnostic",
    poolMax: 20,
    observedConcurrentWaiters: observedWaiting,
    attempted: 20,
    accepted,
...
test("mixed wave: 1000 syncs plus 50 password logins plus 20 checkouts", async () => {
  ...
  const work = [
    ...
    ...people.slice(0, 50).map(
      (u) => () =>
        call(
          "/v1/auth/password/login",
          {
            identifier: u.phone,
            password: "Synthetic!2026",
          },
          { "x-forwarded-for": u.ip },
        ),
    ),
    ...people.slice(50, 70).map(
      (u) => () =>
        call(
          "/v1/payments/checkout",
          {
