...
describe("checkout → gateway → callback", () => {
  ...
  it("persists Zibal IP allowlist failures and logs only a safe one-line diagnostic", async () => {
    const { access } = await signIn();
    const originalRequest = h.psp.request;
    const errorLog = vi.spyOn(console, "error").mockImplementation(() => undefined);
    h.psp.request = async () => ({
      ok: false,
      result: 115,
      provider: "zibal",
      message: "IP is not registered\r\nin panel\t(secret-free)",
    });

    try {
      const response = await checkout(access, { planId: "m1" });
      expect(response.statusCode).toBe(400);
      expect(response.json()).toMatchObject({ error: "psp_failed" });

      const [payment] = await h.query<{ status: string; psp_result: number; provider: string }>(
        "select status, psp_result, provider from payments order by created_at desc limit 1",
      );
      // The test gateway is intentionally named `fake`; the router owns the
      // provider label, while result 115 exercises the exact Zibal failure path.
      expect(payment).toEqual({ status: "failed", psp_result: 115, provider: "fake" });
      expect(errorLog).toHaveBeenCalledWith(
        "psp request failed",
        expect.objectContaining({
          provider: "fake",
          result: 115,
          message: "IP is not registered in panel (secret-free)",
        }),
      );
    } finally {
      h.psp.request = originalRequest;
