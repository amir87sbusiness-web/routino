...
import { grantInterval, hasSettledGrant, listGrants, readEntitlement } from "../src/services/entitlement.js";
...
describe("grantInterval", () => {
  ...
  it("uses real calendar months, not 30-day approximations", async () => {
    ...
    const e = await grantInterval(h.db, USER, { planId: "m12", months: 12, source: "payment" }, now);
  ...
  it("reports expired once the date passes", async () => {
    await grantInterval(h.db, USER, { planId: "trial", days: 7, source: "trial" }, new Date("2026-07-01T00:00:00Z"));
    expect((await readEntitlement(h.db, USER, new Date("2026-07-20T00:00:00Z"))).status).toBe("expired");
  });
  ...
  it("records an auditable ledger entry for every grant", async () => {
    ...
    await grantInterval(h.db, USER, { planId: "m3", months: 3, source: "payment", paymentId: null, note: "test" }, t1);
