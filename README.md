...
- **فرانت SSR-free:** رابط یک SPA استاتیک است، اما ورود، همگام‌سازی، اشتراک و
  پرداخت از Edge API روتینو استفاده می‌کنند.
- **ذخیره‌سازی local-first:** محتوای محصول در Dexie/IndexedDB هر دستگاه است؛
  حساب احراز‌شده همان رکوردهای syncable را از طریق custom JWT و جدول عمومی
  `records` بین تعداد نامحدود دستگاه همگام می‌کند. فرانت دسترسی مستقیم Supabase ندارد.
...
نسخه‌ی `dist/` یک SPA استاتیک استاندارد است و در پروداکشن روی Cloudflare Pages
منتشر می‌شود؛ مسیرهای `/v1` باید به Edge API روتینو متصل باشند.
