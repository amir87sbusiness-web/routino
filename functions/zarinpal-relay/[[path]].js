...
const RELAY_PREFIX = "/zarinpal-relay";
...
export async function onRequest({ request }) {
  ...
  if (request.method === "GET" && url.pathname === `${RELAY_PREFIX}/selftest`) return selftest();
  ...
  try {
    ...
    return json({ error: err?.name === "AbortError" ? "upstream_timeout" : "upstream_error" }, 502);
  } finally {
    clearTimeout(timer);
  }
}
