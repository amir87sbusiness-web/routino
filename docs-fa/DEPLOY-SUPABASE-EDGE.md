...
4. پیش از migration فقط `health/ready`، endpoint read-only مثل `/v1/plans` و fail-closed ماندن outbox را canary کن. فقط بعد از شاهد backup و با مجوز جداگانه، migration افزایشی `20260905140000_elastic_launch_hardening.sql` را اجرا کن: این migration cursor gate و index/functionهای maintenance را اضافه و cron compactor را هر دقیقه با batch حداکثر ۱٬۰۰۰ و tombstone purge را هر ۵ دقیقه با batch حداکثر ۲٬۰۰۰ جایگزین می‌کند؛ هر job `statement_timeout=45s`، `lock_timeout=1s` و advisory no-overlap دارد، ولی خود migration هیچ compaction یا purgeای اجرا نمی‌کند. سپس sync cursor صفر/غیرصفر و non-2xx شدن legacy push برای حسابی با `gc_seq>0` را canary کن. عدد ۱٬۴۴۰٬۰۰۰ task/day فقط ظرفیت نظری schedule×batch است، نه تضمین throughput production.
