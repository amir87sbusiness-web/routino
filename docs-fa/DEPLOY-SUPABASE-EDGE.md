...
|---|---|---|---|
| ~~۱~~ | ~~**کاوه‌نگار**~~ | ✅ **مالک وصل کرد** (۱۴ مرداد ۱۴۰۵) | تأییدش: در لاگ تابع دیگر نباید خط `[!] TEST MODE — SMS` باشد |
| ۲ | **مرچنت واقعی زیبال** (کد پذیرنده) | مالک (احراز کسب‌وکار — درخواست داده شده) | 🔴 پول به حساب نمی‌رسد؛ قبل از تمام‌شدن ۷ روزِ اولین کاربرها لازم است |
| ۳ | اجرای `supabase/setup.sql` در **SQL Editor** پنل Supabase | مالک | 🟡 **دو چیز با هم**: سه ایندکس (`devices_refresh`، `otp_recent`، `payments_discount`) و جاب هفتگی `routino-devices-purge`. مهم‌ترینش `devices_refresh` است: بدون آن **هر بار تازه‌سازی توکن کل جدول `devices` اسکن می‌شود** — یعنی کندی و مصرف CPU روی پلن رایگان. بی‌خطر و قابل اجرای دوباره |
| ۴ | بعد از ۲: `supabase secrets unset ALLOW_TEST_PROVIDERS` + redeploy | Claude می‌تواند | 🟡 برگرداندن محافظ‌های پروداکشن |
...
عددها حدس نیستند — `supabase/tests/quota.test.ts` یک Postgres واقعی (PGlite) را
با ۳۰۰ کاربر شبیه‌سازی‌شده پر می‌کند (ثبت‌نام، تازه‌سازی توکن، خرید پلن، ورود
ناموفق) و بعد اندازه‌ی واقعی جدول‌ها را از خود Postgres می‌پرسد:

| سهمیه‌ی رایگان | مصرف واقعی | ظرفیت |
|---|---|---|
| دیتابیس ۵۰۰ مگابایت | **~۱٫۷ کیلوبایت به ازای هر کاربر، برای همیشه** | **~۳۰۰٬۰۰۰ کاربر** |
| Egress ۵ گیگابایت در ماه | **~۴۸۰ بایت به ازای هر کاربر در روز** | **~۳۷۰٬۰۰۰ کاربر فعال روزانه** |
| ۵۰۰٬۰۰۰ فراخوانی تابع در ماه | ~۳ تا ۴ فراخوانی به ازای هر کاربر در روز | **~۴٬۰۰۰ کاربر فعال روزانه** ← تنگ‌ترین سهمیه |

**پس تنگنای واقعی «تعداد فراخوانی» است، نه حجم.** با ~۴٬۰۰۰ کاربر فعال روزانه
به سقف می‌خوری — که خیلی جلوتر از آن، درآمد اشتراک هزینه‌ی پلن Pro (۲۵ دلار،
سقف ۲ میلیون فراخوانی) را چند برابر پوشش می‌دهد.

**اگر روزی نزدیک سقف شدی، دو اهرم هست** (الان لازم نیستند و عمداً انجام نشده‌اند):
1. `GET /v1/subscriptions/me` الان **هر بار باز شدن اپ** صدا زده می‌شود. می‌شود
   وقتی اشتراک کش‌شده هنوز روزها اعتبار دارد از آن رد شد و مثلاً روزی یک بار
   پرسید. **عمداً دست نخورده:** این مسیر همان جایی است که `meta.tampered` را پاک
   می‌کند و باگ در آن یعنی «هیچ‌کس نمی‌تواند اپ را باز کند».
2. اگر سرویس مانیتورینگ روی `/health` داری، فاصله‌اش را از ۱ دقیقه به ۵ دقیقه
   ببر: ۱ دقیقه یعنی ۴۳٬۲۰۰ فراخوانی در ماه (~۹٪ سهمیه) فقط برای پینگ.

**آنچه این تعادل را به هم می‌زند: روشن‌کردن سینک.** جدول `records` (که هنوز خالی
است) قرار است روزی همه‌ی عادت‌ها و لاگ‌های همه‌ی کاربرها را نگه دارد. آن روز
۱٫۷ کیلوبایت هر کاربر می‌شود چند صد کیلوبایت و ۳۰۰٬۰۰۰ کاربر می‌شود چند هزار.
تست بالا اگر `records` شروع به رشد کند **fail می‌شود** — عمداً، تا این تصمیم
بی‌صدا گرفته نشود.
