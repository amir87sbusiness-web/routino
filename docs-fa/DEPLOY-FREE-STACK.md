...
- [ ] `https://routino.me` باز می‌شود و **صفحه‌ی معرفی** می‌آید (نه خود اپ).
- [ ] `https://routino.me/app/` باز می‌شود و اپ بالا می‌آید.
- [ ] `https://routino.me/app/habits` را مستقیم باز کن — نباید ۴۰۴ بدهد و
      نباید صفحه‌ی معرفی را نشان بدهد. این کارِ Pages Function در
      `functions/app/[[path]].js` است (`_redirects` این کار را نکرد).
...
## عیب‌یابی: «بعد از آپدیت، سایت سفید می‌شود / بالا نمی‌آید»

اگر در کنسول مرورگر این خطا را دیدی:
...
یعنی مرورگر یک فایلِ `assets/app-XXXX.js` خواسته و به‌جایش `index.html` (متنِ HTML)
گرفته. **این باگ کد یا دیپلوی نیست؛ مشکلِ کش است.**

**چرا رخ می‌دهد؟** درست بعد از هر Deploy، اگر یک فایلِ هش‌دار لحظه‌ای هنوز به لبه‌ی
شبکه‌ی کاربر نرسیده باشد، Cloudflare به‌خاطر قانون SPA در `_redirects` همان
`index.html` را با کد ۲۰۰ برمی‌گرداند. اگر آن پاسخ با کش طولانی ذخیره شود، زیر
آدرسِ آن فایل JS «یخ» می‌زند و اپ تا پاک‌شدنِ دستیِ کش بالا نمی‌آید.

**جلوگیری (انجام شده):** در `public/_headers`، هدرِ `assets/*` روی `no-cache` است
(نه `immutable`). این‌طور مرورگر همیشه اعتبار فایل را چک می‌کند، پاسخِ اشتباه هیچ‌وقت
برای همیشه یخ نمی‌زند، و با اولین لودِ بعدی خودش اصلاح می‌شود. **این را به `immutable`
برنگردان** — همان دام را دوباره باز می‌کند. (سرعت افت نمی‌کند چون سرویس‌ورکرِ PWA
اسست‌ها را خودش precache می‌کند.)

**اگر همین حالا گیر کرده‌ای (کشِ قدیمیِ سمی):**
1. یک بار در Cloudflare → پروژه‌ی `routino.me` → **Caching → Purge Everything** بزن.
2. روی دستگاهِ کاربر: مرورگر → DevTools → **Application → Clear site data**، بعد
   `Ctrl+Shift+R`. (روی موبایل: تنظیماتِ سایت → Clear & reset.) یا یک بار در پنجره‌ی
   ناشناس باز کن تا تأیید شود مشکل فقط کشِ همان دستگاه بوده.
...
| `PSP_PROVIDER` | `zibal` یا `zarinpal` | وقتی فقط **یک** درگاه داری |
| `PSP_PROVIDERS` | مثلاً `zarinpal,zibal` | وقتی **هر دو** درگاه رو داری (اختیاری؛ سرور سریع‌ترین رو خودش انتخاب می‌کنه) |
| `ZIBAL_MERCHANT` | کد مرچنت واقعی زیبال | اگه زیبال فعاله |
| `ZARINPAL_MERCHANT` | کد مرچنت واقعی زرین‌پال (۳۶ نویسه) | اگه زرین‌پال فعاله |
...
## چند درگاه و انتخاب خودکار سریع‌ترین

از قبل توی کد آماده است — لازم نیست چیزی بسازی:

- **درگاه‌های موجود:** `zibal`، `zarinpal` و `fake` (فقط توسعه) در
  `backend/src/providers/psp/`.
- **فقط یک درگاه داری؟** `PSP_PROVIDER=zibal` یا `PSP_PROVIDER=zarinpal`. همون
  همیشه استفاده میشه — «یکیش کافیه».
- **هر دو درگاه رو داری؟** `PSP_PROVIDERS=zarinpal,zibal`. حالا سرور برای هر
  پرداخت **سریع‌ترین درگاهِ سالم** رو (از روی زمان پاسخ واقعیِ اخیر) انتخاب
  می‌کنه، و اگه یکی لحظه‌ی پرداخت جواب نداد **خودکار می‌ره سراغ بعدی**. ترتیبِ
  لیست فقط تا وقتیه که هنوز سرعت‌ها اندازه‌گیری نشده.
- روی هر پرداخت نوشته میشه با کدوم درگاه انجام شده، پس تأیید و بازگشت همیشه به
  همون درگاه برمی‌گرده — قاطی نمی‌کنن.
- **درگاه سوم؟** یک فایل تازه کنار همین‌ها بساز + یک خط در
  `backend/src/providers/psp/index.ts` (اکسپورت) و `backend/src/index.ts`
  (ساخت). همون قرارداد `PspProvider` رو پیاده کن؛ روتر خودش تحویلش می‌گیره.

> ⚠️ **درباره‌ی مبلغ زرین‌پال:** آداپتور زرین‌پال مبلغ رو به **ریال** و با
> `currency: "IRR"` می‌فرسته (مثل زیبال و مثل چیزی که کل اپ باهاش قیمت‌گذاری
> می‌کنه). طبق چک‌لیست، **اولین تراکنش واقعی زرین‌پال رو حتماً با کارت خودت بزن**
> و در پنل ادمین چک کن که مبلغ درست کم شده — اشتباه در واحد پول یعنی ۱۰ برابر.
