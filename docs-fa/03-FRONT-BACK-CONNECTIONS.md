...
| ۵ج  | `routes/activation.tsx` — لمس صریح «شروع رایگان»            | `auth.ts` ← `startTrial`                               | `POST /v1/subscriptions/trial/start`                                         | `services/entitlement.ts` — تریال ۷روزهٔ فقط یک‌باره با transaction و قفل ردیف user |
...
| ۷   | `routes/subscribe.tsx` — موقع باز شدن؛ `activation.tsx` — فقط بعد از بازشدن جزئیات پلن‌ها | `payments.ts` ← `fetchPlans` | `GET /v1/plans` | `backend/src/routes/plans.ts` |
