...
**مسیر ثابت-IP زیبال:** کاربر همچنان فقط `api.routino.me` (Cloudflare Worker) را می‌بیند. Supabase قیمت و مجوز را کنترل می‌کند، اما تماس‌های `request` و `verify` را با HMAC به `payment-relay/` می‌فرستد؛ رله merchant را از secret خودش جایگزین و درخواست را با IPv4 ثابت به زیبال می‌فرستد. این رله دیتابیس یا polling ندارد و فقط هنگام پرداخت مصرف ایجاد می‌کند. کد `115` زیبال یعنی IP خروجی رله هنوز در پنل درگاه ثبت نشده است.
