...
| `provider_busy` (+هدر `Retry-After` و همان `paymentId`)                                    | `services/payment-flow.ts`                                 | `subscribe.tsx`                           | ظرفیت لحظه‌ای درگاه پر است؛ همان `attemptId` با مکث کوتاه retry می‌شود و payment تازه ساخته نمی‌شود                   |
