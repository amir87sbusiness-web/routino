...
- عادت‌ها/کارها/ژورنال/تایمر/لاگ/دسته‌ها → اول **IndexedDB** داخل خود دستگاه و بعد فقط deltaهای همان شش نوع از API به جدول `records` می‌روند.
...
- **Sync: IMPLEMENTED.** `POST /v1/sync/exchange`, logic in `services/sync.ts` + `sync-record-validation.ts` (shared, parity-tested), client engine in `src/lib/sync/`. هر exchange ابتدا push و سپس pull را با cursor اولیه در همان invocation انجام می‌دهد تا تغییر دستگاه دیگر جا نیفتد. هر write از `users.seq` شماره می‌گیرد؛ LWW با زمان کلاینتِ clamp‌شده، tombstone، cascade حذف عادت، reset پس از GC و chunking پابرجا هستند. شش kind مجاز دقیقاً `categories/habits/logs/tasks/timerSessions/journal` است؛ payload هرکدام strict و bounded است و تنظیمات در مرز merge/transport پذیرفته نمی‌شوند. semantic-invalidها با metadata محدود `rejectedRecords` جدا می‌شوند، متن خصوصی echo نمی‌شود، validهای همان batch ادامه می‌یابند و کلاینت فقط نسخهٔ پذیرفته/stale را از outbox پاک می‌کند. triggerها: boot، foreground، ۱۰ ثانیه بعد از آخرین ویرایش، و flush فوری best-effort روی hidden/pagehide/background نیتیو؛ foreground تمیزِ تکراری در همان پنجره coalesce می‌شود و poll دوره‌ای وجود ندارد. ورود تازه یک pull اجباری و محدود دیگر بعد از ۲ ثانیه دارد تا تغییر هم‌زمان دستگاه قبلی زود برسد.
...
- **Free-tier budget (re-measured with sync ON — `supabase/tests/quota.test.ts`):** synced record storage remains the non-resetting long-term constraint. Egress is small because exchange فقط deltaها و شش نوع دادهٔ اصلی را حمل می‌کند. Invocation model جدید برای steady-state روزانه ۲ درخواست به‌ازای هر DAU است (boot + یک نشست دارای تغییر)، یعنی سقف ۵۰۰هزار ماهانه حدود ۸۳۳۳ DAU قبل از حاشیهٔ اطمینان می‌دهد؛ تست کف محافظه‌کارانهٔ بیش از ۸۰۰۰ را enforce می‌کند. هیچ refresh، ping دستگاه یا poll دوره‌ای در این عدد نیست.
