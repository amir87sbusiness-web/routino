...
- کلاینت `src/lib/sync/engine.ts` را بعد از تأیید نشست و ثبت تغییرهای محلی اجرا می‌کند: push-before-pull، cursor، LWW، tombstone و reset همگی فعال‌اند. `POST /v1/sync/push` و `GET /v1/sync/pull` فقط auth می‌خواهند، نه اشتراک.
- `records` لاگ sync فعلی است؛ حذف‌ها tombstone می‌مانند تا GC امن با watermark انجام شود.
- تعداد نصب‌های احراز‌هویت‌شده نامحدود است. هر نصب همچنان یک device session با installation key و refresh hash دارد؛ revoke دستی، تغییر رمز و block حساب نشست‌ها را فوراً باطل می‌کنند. lease آفلاین نشست ۱۵ روز است و هیچ ابطالی دیتای محلی را حذف نمی‌کند.
...
- همه‌ی عادت‌ها/کارها/ژورنال/تنظیمات → اول **IndexedDB** داخل خود دستگاه (فایل‌های `src/lib/db/`) و بعد، برای حساب احراز‌شده، deltaهای syncable از API به جدول عمومی `records` می‌روند.
- چیزهای مخصوص همین دستگاه (تم، اعلان‌ها، وضعیت ورود، اشتراکِ کش‌شده) → **localStorage**
- سرور برای ورود/نشست، اشتراک و پرداخت و همچنین بازیابی/همگام‌سازی چنددستگاهی لازم است؛ UI و یادآورهای محلی بدون اینترنت با همان کپی دستگاه کار می‌کنند.
...
| فایل                                                       | آدرس‌ها                                                                                                                                          | نقش                                                                                                                                                                                               |
| ---------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [auth.ts](../backend/src/routes/auth.ts)                   | `/v1/auth/otp/request` `/verify` · **`/password/login`** · **`/account` (GET)** · **`/username`** · **`/password`** · `/token/refresh` `/logout` | ورود با رمز و مسیر OTP برای ثبت‌نام/بازیابی رمز (کد ۴ رقمی؛ بازیابی نشست‌های دیگر را می‌بندد). حساب تازه entitlement=`none` دارد؛ ساخت حساب به‌خودی‌خود grant نمی‌دهد.                            |
...
| [tokens.ts](../backend/src/services/tokens.ts)           | توکن ورود: access (پیش‌فرض ۱ ساعت) + refresh (۱۸۰ روز، با هر بار استفاده عوض می‌شود) + revoke نشست دستگاه                                                                    | مدت‌ها در `env.ts`؛ تعداد نصب‌ها نامحدود است                       |
...
- **Password auth:** phone-first UI defaults to password (`/auth/password/login`, identifier = canonical phone OR lowercased username — disambiguated by whether `normalizePhone` succeeds, since usernames must start with a letter). Hash = scrypt (`services/password.ts`, edge-safe). Brute-force limits per-identifier(8)/per-IP(50) over `login_attempts`; generic `bad_credentials` + DUMMY_HASH verify on miss = no user enumeration. Set username/password from settings when signed in (`/auth/username`, `/auth/password`); password change revokes other devices. NOTE: `supabase/functions/api/routes/*` is hand-written, NOT generated — route changes are mirrored there; only `shared/` comes from `npm run sync:edge`. Admin «تنظیم/ریست رمز» can create an account but grants no access. `OWNER_PHONE`/`OWNER_PASSWORD` bootstrap remains the intentional exception and gives its explicit 12-month owner grant.
- **Device sessions:** authenticated installations are unlimited. The unique `(user_id, installation_key_hash)` row, rotating refresh hash and JWT `did` keep each session identifiable; middleware checks the device row on every private request. Manual revoke, password-change `revokeOtherDevices`, and account block still take effect immediately. Tests: `backend/test/device-security.test.ts`, `backend/test/devices.test.ts`, `src/lib/security-session.test.ts`.

- **Current local-first/cloud-sync budget:** personal records are locally durable first and sync through `records`; the quota test models 48 small records per user and measures the storage slope. Normal visible security checks use `/devices/ping`; sync’s final pull carries entitlement, `/health` is answered at the Cloudflare edge, and only public `/plans` is cached. `supabase/tests/quota.test.ts` measures this production-like path.
