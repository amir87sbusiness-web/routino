...
| [src/lib/backup.ts](../src/lib/backup.ts) | خروجی گرفتن از کل دیتا (فایل JSON) | اسم فایل پشتیبان |
...
| `ui/` (پوشه) | کامپوننت‌های آماده shadcn — بیشترشون استفاده نمی‌شن؛ اپ از `ui.tsx` استفاده می‌کنه | دست نزن |
