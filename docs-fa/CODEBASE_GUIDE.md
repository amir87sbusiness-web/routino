...
- **Landing (`landing/index.template.html`, built by `scripts/build-landing.mjs`):** static, no framework, no signup form, and by owner decision **no mention of price, subscription or the trial**. Its only job is CTA → `/app/`. `copyShots` publishes only the screenshots the HTML actually references, so an orphaned `landing/shots/*.webp` is not shipped. Dark-only on purpose (`color-scheme: dark`) because the product itself is dark and every clip is dark. Structure is hero → five alternating feature rows → scroll-pinned assembly scene → desktop shot → two ways to get the app → closing banner. The layout follows ordinary product-site convention; colour, copy, logo and every claim are ours. **No fabricated social proof:** where such sites put "5M downloads" and a wall of user reviews, this page carries a `.facts` strip of true product statements instead — do not put a number there until a real one exists. **Every screen shown is real footage of the app.** An earlier version rebuilt the screens in HTML/CSS (`.sim`), which looked right but silently went stale whenever the real UI changed. Now `scripts/film-landing.mjs` drives the actual app with Playwright — it ticks a task, scrolls to the habits, ticks one, dismisses the app's own celebration modal, and scrolls back so the day ring is visibly higher (55→75%) — and captures frames into animated WebP (`npm run film`). Both it and `shoot-landing.mjs` share the demo seed in `scripts/lib/app-seed.mjs`. Five traps, all of which failed silently at least once: (1) Animated WebP, not video: Playwright only emits WebM/VP8, which Safari and iOS handle poorly, and transcoding to MP4 needs an ffmpeg that is not installed; animated WebP works from Safari 14 and degrades to its first frame. (2) In sharp, `pageHeight` must go **inside** the `raw` option — outside it, sharp writes one tall static image and throws nothing. (3) libvips **coalesces identical consecutive frames** into one longer-duration frame, so `pages` < frames captured is correct compression, NOT truncation; a very low page count instead means the click selector missed and you filmed a motionless screen, which is what `encode()` now asserts on. (4) Habits and tasks use identical button markup (`h-10 w-10 rounded-full border-2`) and tasks come first in the DOM, so the habit tap must be scoped to the habits `section` or it silently hits a task and the ring never moves. (5) Each row ships a 13–19KB still and fetches its 30–218KB clip only when the row enters view, swapping it in **after** `onload`; swapping `src` on sight leaves the frame blank for the whole download. Clips total 612KB but never load together. Animated WebP has no `pause()`, so the IntersectionObserver, `visibilitychange` and `prefers-reduced-motion` all "pause" by swapping the clip back for its still. The Android CTA deliberately has no `href` and carries `aria-disabled` plus a «به‌زودی» tag — **do not invent an APK URL**; give `#android-btn` a real `href` and drop `soon` when the owner supplies the file. Nothing is visible-by-default-inverted: elements render visible and only the `.js` class (inline `head` script) hides them so animation can run, so a JS failure degrades to a fully readable page, never a blank one. A scroll-pinned "assembly" section (`#assembly`) pins the phone with `position: sticky` while five glass chips fly in and dock onto it, driven by one rAF-throttled scroll handler that writes only `transform`/`opacity` — deliberately no GSAP or Tailwind, since sticky does the pinning for free and ScrollTrigger would add ~70KB to a 43KB page. Three traps there: chip `data-screen` values must ascend or the phone jumps backwards mid-scene; chips must fade to fully 0 after docking or five translucent ghosts stack up at the end; and centring uses physical `top/left`, never `inset-inline-*`, because in RTL the logical value resolves to `right` and `translate(-50%)` then throws the chip left of centre (which clipped them off-screen on mobile). A fourth trap: every chip's `data-at` must be ≥ 0.26 (the lead-in window), or its entrance starts before the section pins and the visitor only ever sees it already docked. Travel distance is computed from real viewport width by `fit()`, not a breakpoint. Without JS or under reduced motion the `live` class is never added and the whole section degrades to a static readable block. The hero clip is `fetchpriority=high` and not lazy, since it is the LCP.
