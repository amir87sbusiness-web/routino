...
- **Quota wait contract:** رد سهمیه فقط همان نسخه را با `account_quota_exceeded` و `retryAt` برمی‌گرداند؛ کلاینت آن را `dirty: 2` نگه می‌دارد و تا موعد retry طوفان درخواست نمی‌سازد. reset فقط ردیف‌های clean را پاک می‌کند، پس دادهٔ محلی/outbox حذف نمی‌شود. خطای DB ناشناخته همچنان ۵۰۰ است و quota جا زده نمی‌شود.
- **Task compaction:** فقط task تکمیل‌شده پس از پایان ماه +۷ روز و آخرین ویرایش +۷ روز، در chunk حداکثر ۳۲ task/۹۶KiB envelope بازشده به `taskMonths` immutable تبدیل می‌شود. cron داخل DB است و صفر invocation اپ می‌سازد؛ ۵۰٬۰۰۰-row cap و annual allowance را دور نمی‌زند. rollout: code first → backup/dry-run/additive migration/canary → cron last.
