...
- **Free-tier budget (measured, not promised — `supabase/tests/quota.test.ts`):** fixture واقعی هر حساب 15 عادت + 10 کار تکمیل‌شده در روز + ژورنال روزانهٔ 7خطی را از Edge می‌فرستد، compaction واقعی را اجرا می‌کند و table/index bytes، تعداد ردیف، annual growth و egress بازشده را چاپ می‌کند. steady state هنوز ۲ invocation/day است؛ روی سقف فرضی ۵۰۰هزار invocation ماهانه حدود ۸۳۳۳ DAU و بدون refresh/ping/poll.
