...
- **Free-tier budget (measured, not assumed — `supabase/tests/quota.test.ts`):** ~1.7 KB permanent DB per user and ~480 B/user/day egress, i.e. ~300k users against 500 MB and ~370k DAU against 5 GB/mo. Both are irrelevant next to the real ceiling: **500k function invocations/mo ≈ 4k DAU**, since every app open costs a `GET /v1/subscriptions/me` plus a token refresh. The test measures a SLOPE (sizes at N/2 vs N) because an empty schema already costs ~250 KB in minimum heap+index pages, which reads as 6 KB/user at test N. It fails if `records` starts growing — turning on sync is the one change that invalidates all of these numbers.
