...
- تعداد نصب‌های احراز‌هویت‌شده نامحدود است و هیچ دستگاه یا نشست سروری ذخیره نمی‌شود. JWT سقف عادی ۳۰روزه دارد و برای حساب قابل‌پاک‌سازی به موعد مؤثر حذف محدود می‌شود؛ خروج محلی است و تغییر رمز توکن‌های قبلی را زودتر باطل نمی‌کند.
...
| [env.ts](../backend/src/env.ts)             | 📌 **لیست همه متغیرهای محیطی** + مقدارهای پیش‌فرض: پورت، `SMS_PROVIDER`، `PSP_PROVIDER`، `ADMIN_PHONE`، `ADMIN_SESSION_SECRET`، `JWT_SECRET`، مدت کد OTP (۱۲۰ ثانیه)، `CORS_ORIGINS`، آدرس‌های عمومی و... | مقدارهای پیش‌فرض؛ در پروداکشن حتماً از env واقعی                                  |
...
| [admin.ts](../backend/src/routes/admin.ts)                 | `/v1/admin/*`                                                                                                       | API ادمین: ورود OTP مالک، نشست cookie امن، آمار، جستجوی کاربر، هدیه‌دادن اشتراک، لیست پرداخت‌ها و ساخت/ویرایش کد تخفیف                                                                            |
...
| [otp.ts](../backend/src/services/otp.ts)                             | کدهای پیامکی: ۴ رقمی، ۲ دقیقه اعتبار، ۳ بار تلاش. سقف هر شماره ۱/دقیقه، ۵/ساعت، ۱۰/روز و هر IP ۲۰/ساعت؛ سقف روزانهٔ سراسری ندارد. تماس هم‌زمان provider با lease محدود است   | ظرفیت فنی در `SMS_PROVIDER_MAX_CONCURRENCY` است، نه تعداد کاربر |
| [provider-capacity.ts](../backend/src/services/provider-capacity.ts) | lease کوتاه و anonymous برای تماس واقعی SMS/PSP؛ ردیف منقضی خودکار قابل‌جایگزینی است و هیچ phone/user/payment ذخیره نمی‌کند                                                  | پس‌فشار فنی است، نه سقف فروش یا ثبت‌نام                         |
...
- پنل ادمین: `http://localhost:3000/admin` — برای تست OTP، `ADMIN_PHONE` را در env محلی بگذار؛ شماره و کد را در فرم وارد کن.
...
- **Password auth:** phone-first UI defaults to password (`/auth/password/login`, identifier = canonical phone OR lowercased username — disambiguated by whether `normalizePhone` succeeds, since usernames must start with a letter). Hash = scrypt (`services/password.ts`, edge-safe). Brute-force limits use fixed-window rows in `auth_rate_limit_buckets`: failures for one opaque key/window increment one row instead of appending forever; generic `bad_credentials` + DUMMY_HASH verify on miss = no user enumeration. Set username/password from settings when signed in (`/auth/username`, `/auth/password`); تغییر رمز JWTهای قبلی را باطل نمی‌کند. NOTE: `supabase/functions/api/routes/*` is hand-written, NOT generated — route changes are mirrored there; only `shared/` comes from `npm run sync:edge`. Admin «تنظیم/ریست رمز» can create an account but grants no access. `OWNER_PHONE`/`OWNER_PASSWORD` bootstrap remains the intentional exception and gives its explicit 12-month owner grant.

- **Current local-first/cloud-sync budget:** personal records are locally durable first and sync through `records`; the quota fixture now uses the approved full account-year: 15 habits, 10 tasks/day and one 7-line journal/day, then runs the real task compactor. It reports rows and physical table/index bytes before/after, positive annual JSON growth, one-/five-year expanded first-sync, and two normal exchanges/day. No security ping or periodic poll exists.
...
- **Pages security/cache:** `scripts/build-landing.mjs` generates a baseline CSP/HSTS/nosniff/frame-deny policy for every static page. Stable HTML, manifest and `sw.js` always revalidate; content-hashed `/app/assets/*` and `workbox-*.js` are immutable for one year. Generated `_routes.json` excludes those static assets from metered Pages Functions. The `/app/*` fallback sets the same security baseline itself (Cloudflare does not apply `_headers` to Function responses) and forces `no-cache`.
...
- **Sync v2: IMPLEMENTED.** `POST /v1/sync/exchange` با `protocolVersion: 2`، logic در `services/sync.ts` + `sync-record-validation.ts` (shared, parity-tested) و client engine در `src/lib/sync/`. هر exchange ابتدا push و سپس pull را از cursor اولیه انجام می‌دهد. شش kind عمومی پابرجا هستند؛ `logs` روی wire به `habitMonths` می‌رود و `taskMonths` فقط kind داخلی DB است که پیش از پاسخ، bounded و lossless به taskهای معمولی باز می‌شود. task جدیدترِ همان id روی archive غلبه می‌کند. نمودار/جست‌وجو/Export/آفلاین هیچ archive client-side ندارند. tombstone، clock clamp، reset، chunking UTF-8، `rejectedRecords` بدون echo و تسویهٔ دقیق outbox پابرجا هستند.
...
- **Throttling:** there is no HTTP rate limiter in production (the edge app has none; Fastify registers `@fastify/rate-limit` for the Node deployment only). Limits are durable in Postgres: `otp_codes` bounds actual SMS spend and `auth_rate_limit_buckets` aggregates password/IP/admin-OTP attempts into one expiring row per key/window. Unknown identifiers never create identifier rows. Two-tier password limits still let the correct password through after the soft limit, preventing attacker-made account lockout. Admin OTP request has an independent IP bucket and reveals neither the configured phone nor whether a submitted number matched.
...
- **اندازه‌گیری ظرفیت (نه وعدهٔ پلن — `supabase/tests/quota.test.ts`):** fixture واقعی هر حساب 15 عادت + 10 کار تکمیل‌شده در روز + ژورنال روزانهٔ 7خطی را از Edge می‌فرستد، compaction واقعی را اجرا می‌کند و table/index bytes، تعداد ردیف، annual growth و egress بازشده را چاپ می‌کند. steady stateِ اندازه‌گیری‌شده دو exchange در روز و بدون refresh/ping/poll است؛ این عدد و bytes چاپ‌شده فقط خروجی همین fixture هستند، نه تضمین تعداد کاربر یا سهمیهٔ Supabase.
- **Per-account abuse budget:** `sync_record_count` سقف سخت ۵۰٬۰۰۰ ردیف دارد و `sync_data_bytes` فقط شمارندهٔ دقیق و غیرمنفی حجم جاری است؛ سقف مادام‌العمر ۱۲۸ MiB حذف شده. write budget برابر ۱۰ MiB رشد مثبت JSON در دورهٔ ۳۶۵روزهٔ هر حساب است. migration دادهٔ موجود را grandfather می‌کند. shrink/delete مجاز است ولی سهمیه پس نمی‌دهد؛ با شروع دورهٔ بعد شمارنده از صفر آغاز می‌شود.
- **Quota wait contract:** رد سهمیه فقط همان نسخه را با `account_quota_exceeded` و `retryAt` برمی‌گرداند؛ رشد سالانه retryAt پایان دقیق دوره و row cap retryAt محدود روزانه دارد. کلاینت آن را `dirty: 2` نگه می‌دارد و تا موعد retry طوفان درخواست نمی‌سازد. reset فقط ردیف‌های clean را پاک می‌کند، پس دادهٔ محلی/outbox حذف نمی‌شود. خطای DB ناشناخته همچنان ۵۰۰ است و quota جا زده نمی‌شود.
- **Task compaction:** فقط task تکمیل‌شده پس از پایان ماه +۷ روز و آخرین ویرایش +۷ روز، در chunk حداکثر ۳۲ task/۹۶KiB envelope بازشده به `taskMonths` immutable تبدیل می‌شود. cron داخل DB است و صفر invocation اپ می‌سازد؛ owner پیش از source task claim می‌شود و cron با transaction-local `statement_timeout='5000ms'` و `lock_timeout='1000ms'` محدود می‌شود. ۵۰٬۰۰۰-row cap و annual allowance را دور نمی‌زند. rollout: code first → backup/dry-run/additive migration/canary → cron last.
- **Trial account retention:** فقط registration-only یا یک تریال سازگارِ ۷روزه و بدون هر نوع سابقه مالی/ادمین/discount reference واجد حذف است. deadline دیرترینِ `created_at+30d`، پایان تریال و برای کاربران قدیمی `deployTime+30d` است. cleanup روزانه batch=50، idempotent، `SKIP LOCKED` و timeout-bounded است؛ `records`/`taskMonths` با cascade حذف می‌شوند. تا وقتی حساب واقعاً حذف نشده، registration-only و trial-only فعال/منقضی در فهرست، جستجو و جزئیات ادمین مثل همهٔ کاربران دیده می‌شوند و grant ادمین می‌گیرند. شمارندهٔ ناشناس `trial_starts` فقط متریک اضافه است.
- **Tombstone GC is scheduled** (`routino-tombstone-purge`, every 5 minutes, batch ≤2000, in `scripts/gen-setup-sql.mjs` → `supabase/setup.sql`): it deletes only `records` tombstones older than 90 days and raises `users.gc_seq` in the same transaction. An old device gets a read-only resumable full-resync instead of resurrecting deleted data. The job has 45s statement/1s lock timeouts and no-overlap; `cron.job_run_details` must be monitored because local fixture throughput is not production proof.
