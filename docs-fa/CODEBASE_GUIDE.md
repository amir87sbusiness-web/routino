...
- **Landing (`landing/index.template.html`, built by `scripts/build-landing.mjs`):** static, no framework, no signup form, and by owner decision **no mention of price, subscription or the trial**. Its only job is CTA → `/app/`. `copyShots` publishes only the screenshots the HTML actually references, so an orphaned `landing/shots/*.webp` is not shipped. Dark-only on purpose (`color-scheme: dark`) because the product itself is dark and every shot is dark. Its palette and motion are copied from the app, not invented: color tokens come from the `.dark` block in `src/styles.css` (converted oklch→hex; the real primary is `#fe7825`, not `#f97316`), and the `page-item-in` / `check-pop` / `row-complete-flash` / `pop-in` keyframes plus the 60ms `page-stagger` ladder are reproduced verbatim, so the site moves the way the product moves — re-copy them if the app's theme changes. The Android CTA is deliberately a `span` with `aria-disabled` and a «به‌زودی» tag and **must not be given a guessed APK URL**; swap it for `<a href download>` when the owner supplies the file. Structure is hero → "console" → spec band → desktop shot → 3 install steps → close; the console deliberately replaced five stacked feature rows. **The console contains no images at all**: the app's screens are rebuilt in HTML/CSS (`.sim`) and a JS player animates real interactions (a habit is tapped, the day ring fills 60→75%, the timer counts, charts grow), looping through five scenes that the `role=tablist` rail also jumps to. That recreation is why `landing/shots/` dropped from 7 published images to 3 — but it also means **a UI change in the real app does not propagate here; it must be re-done by hand** or the site advertises a screen that no longer exists. Four traps: (1) nothing is visible-by-default-inverted — elements render visible and only the `.js` class (inline `head` script) hides them so animation can run, so a JS failure degrades to a fully readable page, never a blank one; the same rule governs `.sim.armed`, which is added only at play time so a player that never starts still shows a full app screen. Do not move either hiding rule into a base selector. (2) `go()` commits the start state with a forced `void offsetWidth` reflow, NOT `requestAnimationFrame` — a throttled or backgrounded tab can skip rAF entirely, which previously left charts stuck at `scaleY(0)`. (3) The tap indicator lives inside the row it taps and is positioned with `inset`, never a hardcoded `top` px, which used to make the finger land on the wrong row. (4) The player stops via IntersectionObserver + `visibilitychange` so it never animates off-screen. On ≤860px the desktop frame is `display:none` with media-scoped `<link rel=preload>` per breakpoint, which is what stops mobile downloading the 64KB desktop shot; that shot is `fetchpriority=high` and NOT lazy, since it is the desktop LCP.
