...
- **Sync: IMPLEMENTED.** `POST /v1/sync/exchange`, logic in `services/sync.ts` (shared, parity-tested), client engine in `src/lib/sync/`. هر exchange ابتدا push و سپس pull را با cursor اولیه در همان invocation انجام می‌دهد تا تغییر دستگاه دیگر جا نیفتد. هر write از `users.seq` شماره می‌گیرد؛ LWW با زمان کلاینتِ clamp‌شده، tombstone، cascade حذف عادت، reset پس از GC، chunking و نگه‌داشتن رکورد ردشده در outbox پابرجا هستند. شش kind مجاز دقیقاً `categories/habits/logs/tasks/timerSessions/journal` است و تنظیمات در مرز merge/transport پذیرفته نمی‌شوند. triggerها: boot، foreground، ۱۰ ثانیه بعد از آخرین ویرایش، و flush فوری best-effort روی hidden/pagehide/background نیتیو؛ foreground تمیزِ تکراری در همان پنجره coalesce می‌شود و poll دوره‌ای وجود ندارد. ورود تازه یک pull اجباری و محدود دیگر بعد از ۲ ثانیه دارد تا تغییر هم‌زمان دستگاه قبلی زود برسد.
