...
>
> آخرین به‌روزرسانی: **۲۷ مرداد ۱۴۰۵ (2026-08-18)**.
>
...
- محتوای شخصی کاربر فقط در vault محلی همان حساب می‌ماند و هیچ sync سروری ندارد.
- کلاینت `src/lib/sync/engine.ts` را اجرا نمی‌کند؛ endpointهای قدیمی sync در production همیشه `410 sync_disabled` هستند و env روشن‌کنندهٔ آن در production ممنوع است.
- `records` فقط برای مهاجرت نسخه‌های قدیمی در schema مانده و بعد از استقرار این نسخه باید خالی شود. توضیحات طولانی sync در انتهای این راهنما صرفاً تاریخچهٔ مهندسی/تست سازگاری‌اند، نه رفتار محصول لانچ.
- پیش‌فرض یک دستگاه فعال است؛ ادمین می‌تواند ۱ تا ۱۰ تعیین کند. سه جایگزینی در rolling 15 days مجاز و چهارمی قفل امنیتی است. lease آفلاین نشست ۱۵ روز است و هیچ قفلی دیتای محلی را حذف نمی‌کند.
- Export همیشه فعال است؛ Import فقط اشتراک پولی فعال (نه trial/free/expired). اعلان پایان اشتراک در ۳ روز مانده و لحظهٔ پایان ساخته می‌شود.
...
| [settings.tsx](../src/routes/settings.tsx) | **تنظیمات**: حساب، زبان/تم، نوتیف، Export/Import و قوانین | Export همیشه؛ Import فقط پولی |
...
| [db/schema.ts](../backend/src/db/schema.ts) | ساختار حساب/دستگاه/امنیت/اشتراک/پرداخت. `records` میراثی و در لانچ بدون writer است | فیلد جدید با DDL + sync:edge |
...
| [otp.ts](../backend/src/services/otp.ts) | کدهای پیامکی: ۴ رقمی، ۲ دقیقه اعتبار، ۵ بار تلاش. **سقف‌های ارسال** (`LIMITS`): هر شماره ۱/دقیقه، ۵/ساعت، ۱۰/روز؛ هر IP ۲۰/ساعت؛ کل سیستم ۲۰۰۰/روز | سقف‌ها رو اینجا عوض کن — اینا جلوی خالی‌شدن شارژ پیامکت رو می‌گیرن |
...
- **Gate:** `AppShell` → onboarding → auth (`db.auth`, device-local) → `subscriptionActive(db)` (local cache + `meta.tampered` check) → app. Device security uses lightweight `GET /v1/devices/ping` on boot/online/foreground and every visible minute. Full entitlement refresh is every 6h normally and every 1h in the final 3 days; it also repairs stranded payments. A brand-new phone gets a 7-day trial on `/auth/otp/verify`.
...
- **Auth tokens:** localStorage `routino:auth:v1` (`lib/api/auth.ts`), access JWT defaults to 1h and the client reads its real `exp` (1h fallback for malformed legacy tokens), opaque rotating refresh; single-flight refresh; offline never signs out. Protected middleware still verifies the user/device row on every request, so revoke does not wait for JWT expiry.
...
- **Device policy:** default active-device limit is 1 and admin can set 1–10 per account. A new device revokes the least-recently-used active device. Three replacements are allowed in a rolling 15-day window; the fourth locks the account with a supportive security message and `@routino_support`. Auth middleware checks the device row on every private request; the client also pings each visible minute. An offline lease lasts 15 days and never deletes local data. Tests: `backend/test/device-security.test.ts`, `backend/test/devices.test.ts`, `src/lib/security-session.test.ts`.

- **Current local-first budget:** personal records never leave IndexedDB and production sync endpoints return `410 sync_disabled`; `records` must remain empty. Normal visible security checks use the small `/devices/ping` response, entitlement reads are 6h/1h cadence, `/health` is answered at the Cloudflare edge, and only public `/plans` is cached. `supabase/tests/quota.test.ts` measures this production-like path.
...
> The detailed sync/cursor/tombstone notes below are retained only as engineering history for legacy migration tests. They do **not** describe the launch product or its current invocation budget.
